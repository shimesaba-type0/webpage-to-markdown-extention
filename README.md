...
この拡張機能は、画像を2つの異なる方法で扱います：

### サイドパネルでの表示

**動作**: オリジナルのURL（元のWebサイトのURL）を使用して画像を表示

**理由**:
- リアルタイムで最新の画像を表示できる
- ストレージ容量を節約（画像をダウンロードしない場合）
- ネットワーク接続がある環境で即座に表示可能

**制限**:
- インターネット接続が必要
- 元のサイトで画像が削除されると表示できなくなる

### エクスポート時の処理

**動作**: ローカルにダウンロードした画像を含むZIPファイルを生成

**内容**:
```
exported-article.zip
├── article.md          # ローカル画像パス（./images/xxx.jpg）を使用
└── images/
    ├── image1.jpg
    ├── image2.png
    └── ...
...
**ZIPファイルの使い方**:
1. ZIPファイルを解凍
2. `article.md` をMarkdownエディタで開く
3. 画像が `./images/` フォルダから読み込まれる

### 画像ダウンロード設定

画像のダウンロードは**デフォルトで無効**になっています（著作権とプライバシー保護のため）。

**有効化の方法**:

**方法1: ポップアップから（推奨）**
1. 拡張機能のポップアップを開く
2. 「Settings」セクションで「Download images for offline viewing」にチェック
3. 設定は即座に保存されます

**方法2: オプションページから**
1. 拡張機能アイコンを右クリック → 「オプション」
2. 「Image Settings」セクションで「Enable Image Download」にチェック
3. 保存ボタンをクリック

**注意事項**:
- ⚠️ ダウンロードした画像は**個人利用のみ**です
- ⚠️ 著作権と利用規約を尊重してください
- ⚠️ 既に保存済みの記事には遡って適用されません（新しく抽出する記事のみ）
...
- **画像表示**: サイドパネルではオリジナルURLを使用（上記「画像の取り扱い」参照）
- **画像ダウンロード**: デフォルトで無効（ユーザーが明示的に有効化する必要があります）
- **翻訳機能**: Phase 4で実装予定
