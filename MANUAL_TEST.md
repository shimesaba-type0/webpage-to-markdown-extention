...
#### 画像ダウンロード設定（ポップアップ）
- [ ] 1. 拡張機能のポップアップを開く
- [ ] 2. 「Settings」セクションに「Download images for offline viewing」チェックボックスが表示される
- [ ] 3. チェックボックスをオン → 「✓ Image download enabled」メッセージが表示される
- [ ] 4. チェックボックスをオフ → 「Image download disabled」メッセージが表示される
- [ ] 5. ポップアップを閉じて再度開く → 設定が保持されている

#### 画像ダウンロード設定（オプションページ）
- [ ] 1. 拡張機能を右クリック → 「オプション」
- [ ] 2. 「Image Settings」セクションが表示される
- [ ] 3. 「Enable Image Download」チェックボックスをオン/オフ
- [ ] 4. 「Save Settings」で保存
- [ ] 5. ポップアップの設定と同期される

#### サイドパネルでの画像表示（ダウンロード無効時）
- [ ] 1. 画像ダウンロードを**無効**にする
- [ ] 2. 画像を含む記事を抽出（例: Wikipedia）
- [ ] 3. サイドパネルで記事を表示
- [ ] 4. **期待**: 画像がオリジナルURL（元サイトのURL）で表示される
- [ ] 5. マークダウンタブで確認: `![alt](https://original-site.com/image.jpg)`

#### サイドパネルでの画像表示（ダウンロード有効時）
- [ ] 1. 画像ダウンロードを**有効**にする
- [ ] 2. 新しい記事を抽出
- [ ] 3. サイドパネルで記事を表示
- [ ] 4. **期待**: 画像が表示される（内部的にBlobURLを使用）
- [ ] 5. マークダウンタブで確認: `![alt](https://original-site.com/image.jpg)` （変わらず）

#### エクスポート機能（ダウンロード有効時）
- [ ] 1. 画像ダウンロードを**有効**にする
- [ ] 2. 画像を含む記事を抽出
- [ ] 3. サイドパネルで「Download」ボタンをクリック
- [ ] 4. ZIPファイルがダウンロードされる
- [ ] 5. ZIPを解凍して確認:
  - `article.md` ファイルが存在
  - `images/` フォルダが存在
  - 画像ファイル（.jpg, .png等）が`images/`内に存在
  - `article.md`内のマークダウンが`./images/xxx.jpg`形式のローカルパス

#### エクスポート機能（ダウンロード無効時）
- [ ] 1. 画像ダウンロードを**無効**にする
- [ ] 2. 記事を抽出
- [ ] 3. サイドパネルで「Download」ボタンをクリック
- [ ] 4. `.md`ファイルのみダウンロードされる（ZIPなし）
- [ ] 5. マークダウンにオリジナルURLが保持されている

#### 画像の動作理解確認
- [ ] **理解**: サイドパネルではオリジナルURLを使用（リアルタイム表示）
- [ ] **理解**: エクスポート時はローカル画像パス（オフライン保存）
- [ ] **理解**: ダウンロード無効時は画像がIndexedDBに保存されない
- [ ] **理解**: 既存の記事には設定変更が遡って適用されない
...
3. **画像の動作仕様**:
   - **サイドパネル**: オリジナルURL使用（リアルタイム表示、ネット接続必要）
   - **エクスポート**: ローカル画像パスとファイル（オフライン閲覧可能）
   - **デフォルト**: 画像ダウンロード無効（著作権とプライバシー保護）
   - **設定変更**: 既存の記事には遡って適用されない
