...
### 1ファイル1コミット原則
プロジェクトrootファイルを削除する際は、必ず1ファイルにつき1コミットを作成すること。
理由：後から特定のファイルのみ `git revert <コミットハッシュ>` で復活させやすくするため。

### 削除フロー（チーム開発時）
1. Developer: 各ファイルの調査レポート作成（目的・参照・削除影響を分析）
2. Reviewer: 独立した視点で削除妥当性を検証し、論理的にYES/NOをレポート
3. Judge（判定者）: Reviewerレポートに基づき最終承認/否認を決定
   - 拡張機能動作への影響が1%でもある → 保持（NO）
   - package.json等からの参照あり → 保持（NO）
   - 判断が微妙 → 保持（NO）
4. Developer: Judge承認後に1ファイル1コミットで削除実行

### DELETION_LOG.mdの管理
rootファイルを削除する際は `DELETION_LOG.md` を作成・更新する。
記録内容：ファイル名、削除日、削除理由、判定者、関連Issue番号。

### 削除コミットメッセージ形式
```
chore: remove <ファイル名> (#Issue番号) - <削除理由>
```
